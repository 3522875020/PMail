build: build_fe build_server telegram_push wechat_push spam_block package
...
build_server:
	cd server && CGO_ENABLED=0 GOOS=linux GOARCH=s390x go build -ldflags "-s -w -X 'main.goVersion=$(go version)' -X 'main.gitHash=$(git show -s --format=%H)' -X 'main.buildTime=$(TZ=UTC-8 date +%Y-%m-%d" "%H:%M:%S)'" -o pmail_linux_s390x  main.go
...
telegram_push:
	...
	cd server/hooks/telegram_push && CGO_ENABLED=0 GOOS=linux GOARCH=s390x go build -ldflags "-s -w" -o output/telegram_push_linux_s390x  telegram_push.go
...
wechat_push:
	...
	cd server/hooks/wechat_push && CGO_ENABLED=0 GOOS=linux GOARCH=s390x go build -ldflags "-s -w" -o output/wechat_push_linux_s390x  wechat_push.go
...
spam_block:
	...
	cd server/hooks/spam_block && CGO_ENABLED=0 GOOS=linux GOARCH=s390x go build -ldflags "-s -w" -o output/spam_block_linux_s390x  spam_block.go
